...
public searchProduct(keyword:string,page:number,size:number) : Observable<PageProduct>{
    let result = this.products.filter(p=>p.name.includes(keyword.toUpperCase()));
  let index = page*size;
...
public getProduct(id :string) : Observable<Product>{
    let product = this.products.find(p => p.id=id );
    if(product == undefined) return throwError(()=>new Error("product nor found"));
    return of(product);
}
